...
class BezierChannel(object):
    ...
    def fit_channel_with_bezier(self):
        ...
        if self.shroud is not None:
            self.control_points_shroud, self.shroud_degrees = self.__fit_with_bezier(
                self.shroud
            )
        else:
    ...
    def __get_from_control_points(self, control_points: np.ndarray, degrees: list):
        ...
        for i in range(n_segments):
            if i != (n_segments - 1):
                upper_bound: int = (
                    lower_bound + 1 if degrees[i] == 1 else lower_bound + degrees[i]
                )
    ...
    def get_channel_from_control_points(self):
        self.fitted_hub = self.__get_from_control_points(
            self.control_points_hub, self.hub_degrees
        )
        self.fitted_shroud = self.__get_from_control_points(
            self.control_points_shroud, self.shroud_degrees
        )
    ...
    def draw_channel(self, show: bool = False, width: int = 1000, height: int = 800):
        fig = go.Figure(
            data=[
                go.Scatter3d(
                    x=self.control_points_hub[:, 0],
                    y=self.control_points_hub[:, 1],
                    z=self.control_points_hub[:, 2],
                    mode="markers",
                    marker=dict(size=5),
                ...
                go.Scatter3d(
                    x=self.control_points_shroud[:, 0],
                    y=self.control_points_shroud[:, 1],
                    z=self.control_points_shroud[:, 2],
                    mode="markers",
                    marker=dict(size=5),
                    name="Control Points Shroud",
                ),
                go.Scatter3d(
                    x=self.fitted_shroud[:, 0],
                    y=self.fitted_shroud[:, 1],
                    z=self.fitted_shroud[:, 2],
                    mode="markers",
                    marker=dict(size=5),
...
class BezierBlade(object):
    ...
    def __fit_section_with_bezier(
        ...
        assert (
            section[0, 0, :] == section[1, -1, :]
        ).all(), "The last point of the suction side point and the pressure side first one must be identical"
        ...
        if self.split_edges:
            if self.is_blunt_te:
                ...
                ss_control_points = np.array(
                    get_bezier_parameters(
                        ss[:, 0], ss[:, 1], ss[:, 2], degree=self.__bezier_degree[0]
                    )
                )
                ...
                if self.record_metrics:
                    ...
                    self.fitting_metrics.append(
                        fitting_metrics(ss[..., :3], np.vstack((xvals, yvals, zvals)).T)
                    )
                ...
                if self.record_metrics:
                    ...
                    self.fitting_metrics.append(
                        fitting_metrics(
                            ss_le[..., :3], np.vstack((xvals, yvals, zvals)).T
                        )
                    )
                ...
                if self.record_metrics:
                    ...
                    self.fitting_metrics.append(
                        fitting_metrics(
                            ps_le[..., :3], np.vstack((xvals, yvals, zvals)).T
                        )
                    )

                ps_control_points = np.array(
                    get_bezier_parameters(
                        ps[:, 0], ps[:, 1], ps[:, 2], degree=self.__bezier_degree[3]
                    )
                )
                ...
                if self.record_metrics:
                    ...
                    self.fitting_metrics.append(
                        fitting_metrics(ps[..., :3], np.vstack((xvals, yvals, zvals)).T)
                    )

                assert (
                    (ss_control_points[-1] == ss_le_control_points[0]).all()
                ...
                return np.concatenate(
                    [
                        ss_control_points,
                        ss_le_control_points,
                        ps_le_control_points,
                        ps_control_points,
                    ],
                    axis=0,
                )
                ...
                ss = section[
                    1, self.__TE_POINTS_NUMBER - 1 : -self.__LE_POINTS_NUMBER, :
                ]
                ss_le = section[1, -self.__LE_POINTS_NUMBER - 1 :, :]
                ps_le = section[0, : self.__LE_POINTS_NUMBER + 1, :]
                ...
                if self.record_metrics:
                    ...
                    self.fitting_metrics.append(
                        fitting_metrics(
                            ss_te[..., :3], np.vstack((xvals, yvals, zvals)).T
                        )
                    )

                ss_control_points = np.array(
                    get_bezier_parameters(
                        ss[:, 0], ss[:, 1], ss[:, 2], degree=self.__bezier_degree[1]
                    )
                )
                ...
                ps_control_points = np.array(
                    get_bezier_parameters(
                        ps[:, 0], ps[:, 1], ps[:, 2], degree=self.__bezier_degree[4]
                    )
                )
                ...
                if self.record_metrics:
                    ...
                    self.fitting_metrics.append(
                        fitting_metrics(
                            ps_te[..., :3], np.vstack((xvals, yvals, zvals)).T
                        )
                    )

                assert (
                    (ss_te_control_points[-1] == ss_control_points[0]).all()
                    and (ss_control_points[-1] == ss_le_control_points[0]).all()
                    and (ss_le_control_points[-1] == ps_le_control_points[0]).all()
                    and (ps_le_control_points[-1] == ps_control_points[0]).all()
                    and (ps_control_points[-1] == ps_te_control_points[0]).all()
                    and (ps_te_control_points[-1] == ss_te_control_points[0]).all()
                ), "Error: please check fit_section_with_bezier function"
                ...
                return np.concatenate(
                    [
                        ss_te_control_points,
                        ss_control_points,
                        ss_le_control_points,
                        ps_le_control_points,
                        ps_control_points,
                        ps_te_control_points,
                    ],
                    axis=0,
                )
            ...
            if self.is_blunt_te:
                ...
                le_control_points = np.array(
                    get_bezier_parameters(
                        le[:, 0], le[:, 1], le[:, 2], degree=self.__bezier_degree[1]
                    )
                )
                ...
                if self.record_metrics:
                    ...
                    self.fitting_metrics.append(
                        fitting_metrics(le[..., :3], np.vstack((xvals, yvals, zvals)).T)
                    )

                ps_control_points = np.array(
                    get_bezier_parameters(
                        ps[:, 0], ps[:, 1], ps[:, 2], degree=self.__bezier_degree[2]
                    )
                )
                ...
                assert (ss_control_points[-1] == le_control_points[0]).all() and (
                    le_control_points[-1] == ps_control_points[0]
                ).all(), "Error: please check fit_section_with_bezier function"
                ...
                return np.concatenate(
                    [ss_control_points, le_control_points, ps_control_points], axis=0
                )
                ...
                te_control_points = np.array(
                    get_bezier_parameters(
                        te[:, 0], te[:, 1], te[:, 2], degree=self.__bezier_degree[3]
                    )
                )
                ...
                if self.record_metrics:
                    ...
                    self.fitting_metrics.append(
                        fitting_metrics(te[..., :3], np.vstack((xvals, yvals, zvals)).T)
                    )

                assert (
                    (ss_control_points[-1] == le_control_points[0]).all()
                    and (le_control_points[-1] == ps_control_points[0]).all()
                    and (ps_control_points[-1] == te_control_points[0]).all()
                    and (te_control_points[-1] == ss_control_points[0]).all()
                ), "Error: please check fit_section_with_bezier function"
        ...
        if self.numpy_blade is not None:
            ...
            for i in range(self.numpy_blade.shape[1]):
                control_points.append(
                    self.__fit_section_with_bezier(self.numpy_blade[:, i, :, :3])
                )
    ...
    def __get_section_from_control_points(self, control_points: np.ndarray):
        if self.split_edges:
            if self.is_blunt_te:
                ...
                ss_le_control_points = control_points[
                    self.__bezier_degree[0]
                    + 1 : self.__bezier_degree[0]
                    + 1
                    + self.__bezier_degree[1]
                ]
                ss_le_control_points = np.insert(
                    ss_le_control_points, 0, ss_control_points[-1], axis=0
                )

                ps_le_control_points = control_points[
                    self.__bezier_degree[0]
                    + 1
                    + self.__bezier_degree[1] : self.__bezier_degree[0]
                    + 1
                    + self.__bezier_degree[1]
                    + self.__bezier_degree[2]
                ]
                ps_le_control_points = np.insert(
                    ps_le_control_points, 0, ss_le_control_points[-1], axis=0
                )

                ps_control_points = control_points[
                    self.__bezier_degree[0]
                    + 1
                    + self.__bezier_degree[1]
                    + self.__bezier_degree[2] :
                ]
                ps_control_points = np.insert(
                    ps_control_points, 0, ps_le_control_points[-1], axis=0
                )

                ss = np.array(
                    bezier_curve(ss_control_points, nTimes=self.__evaluation_points[0])
                ).T
                ss_le = np.array(
                    bezier_curve(
                        ss_le_control_points, nTimes=self.__evaluation_points[1]
                    )
                ).T
                ps_le = np.array(
                    bezier_curve(
                        ps_le_control_points, nTimes=self.__evaluation_points[2]
                    )
                ).T
                ps = np.array(
                    bezier_curve(ps_control_points, nTimes=self.__evaluation_points[3])
                ).T
                ...
                ss_control_points = control_points[
                    self.__bezier_degree[0] : self.__bezier_degree[0]
                    + self.__bezier_degree[1]
                ]
                ss_control_points = np.insert(
                    ss_control_points, 0, ss_te_control_points[-1], axis=0
                )
                ...
                ps_le_control_points = control_points[
                    self.__bezier_degree[0]
                    + self.__bezier_degree[1]
                    + self.__bezier_degree[2] : self.__bezier_degree[0]
                    + self.__bezier_degree[1]
                    + self.__bezier_degree[2]
                    + self.__bezier_degree[3]
                ]
                ps_le_control_points = np.insert(
                    ps_le_control_points, 0, ss_le_control_points[-1], axis=0
                )
                ...
                ps_control_points = control_points[
                    ...
                    + self.__bezier_degree[1]
                    + self.__bezier_degree[2]
                    + self.__bezier_degree[3] : self.__bezier_degree[0]
                    + self.__bezier_degree[1]
                    + self.__bezier_degree[2]
                    + self.__bezier_degree[3]
                    + self.__bezier_degree[4]
                ]
                ps_control_points = np.insert(
                    ps_control_points, 0, ps_le_control_points[-1], axis=0
                )

                ps_te_control_points = control_points[
                    self.__bezier_degree[0]
                    + self.__bezier_degree[1]
                    + self.__bezier_degree[2]
                    + self.__bezier_degree[3]
                ...
                ps_te_control_points = np.insert(
                    ps_te_control_points, 0, ps_control_points[-1], axis=0
                )

                ss_te_control_points = np.insert(
                    ss_te_control_points, 0, ps_te_control_points[-1], axis=0
                )

                ss_te = np.array(
                    bezier_curve(
                        ss_te_control_points, nTimes=self.__evaluation_points[0]
                    )
                ).T
                ss = np.array(
                    bezier_curve(ss_control_points, nTimes=self.__evaluation_points[1])
                ).T
                ss_le = np.array(
                    bezier_curve(
                        ss_le_control_points, nTimes=self.__evaluation_points[2]
                    )
                ).T
                ps_le = np.array(
                    bezier_curve(
                        ps_le_control_points, nTimes=self.__evaluation_points[3]
                    )
                ).T
                ps = np.array(
                    bezier_curve(ps_control_points, nTimes=self.__evaluation_points[4])
                ).T
                ps_te = np.array(
                    bezier_curve(
                        ps_te_control_points, nTimes=self.__evaluation_points[5]
                    )
                ).T
            ...
            if self.is_blunt_te:
                ...
                le_control_points = control_points[
                    self.__bezier_degree[0]
                    + 1 : self.__bezier_degree[0]
                    + 1
                    + self.__bezier_degree[1]
                ]
                le_control_points = np.insert(
                    le_control_points, 0, ss_control_points[-1], axis=0
                )

                ps_control_points = control_points[
                    self.__bezier_degree[0] + 1 + self.__bezier_degree[1] :
                ]
                ps_control_points = np.insert(
                    ps_control_points, 0, le_control_points[-1], axis=0
                )
                ...
                le = np.array(
                    bezier_curve(le_control_points, nTimes=self.__evaluation_points[1])
                ).T
                ps = np.array(
                    bezier_curve(ps_control_points, nTimes=self.__evaluation_points[2])
                ).T
                ...
                te_control_points = control_points[
                    self.__bezier_degree[0]
                    + self.__bezier_degree[1]
                    + self.__bezier_degree[2] :
                ]
                te_control_points = np.insert(
                    te_control_points, 0, ps_control_points[-1], axis=0
                )

                ss_control_points = np.insert(
                    ss_control_points, 0, te_control_points[-1], axis=0
                )
                ...
                te = np.array(
                    bezier_curve(te_control_points, nTimes=self.__evaluation_points[3])
                ).T
    ...
    def get_blade_from_control_points(self):
        if self.control_points is not None:
            ...
            for i in range(self.control_points.shape[0]):
                sections.append(
                    self.__get_section_from_control_points(self.control_points[i])
                )
...
class BezierCompressor(object):
    def __init__(
        self,
        geomturbo_path: str,
        bezier_degree: list | List[list, list, list],
        is_blunt_te: list,
        evaluation_points: list | List[list, list, list],
        split_edges: list,
        is_main_blade_active: bool = True,
        is_splitter_active: bool = True,
        is_diffuser_active: bool = True,
        is_channel_active: bool = True,
        channel_degree: int = 6,
        channel_evaluation_points: int = 30,
        record_metrics: bool = False,
    ):
        ...
        if is_main_blade_active:
            self.main_blade = BezierBlade(
                bezier_degree=self.bezier_degree[0],
                is_blunt_te=self.is_blunt_te[0],
                evaluation_points=self.evaluation_points[0],
                split_edges=split_edges[0],
                record_metrics=record_metrics,
            )
        if is_splitter_active:
            self.splitter = BezierBlade(
                bezier_degree=self.bezier_degree[1],
                is_blunt_te=self.is_blunt_te[1],
                evaluation_points=self.evaluation_points[1],
                split_edges=split_edges[1],
                record_metrics=record_metrics,
            )
        if is_diffuser_active:
            self.diffuser = BezierBlade(
                bezier_degree=self.bezier_degree[2],
                is_blunt_te=self.is_blunt_te[2],
                evaluation_points=self.evaluation_points[2],
                split_edges=split_edges[2],
                record_metrics=record_metrics,
            )
        if is_channel_active:
            self.channel = BezierChannel(
                degree=channel_degree,
                evaluation_points=channel_evaluation_points,
                record_metrics=record_metrics,
            )
    ...
    def set_le_points_number(self, points_number: int | List[int, int, int]) -> None:
        ...
        self.main_blade.set_le_points_number(
            points_number[0]
        ) if self.is_main_blade_active else None
        self.splitter.set_le_points_number(
            points_number[1]
        ) if self.is_splitter_active else None
        self.diffuser.set_le_points_number(
            points_number[2]
        ) if self.is_diffuser_active else None

    def set_te_points_number(self, points_number: int | List[int, int, int]) -> None:
        if isinstance(points_number, int):
            points_number = [points_number for _ in range(3)]
        self.main_blade.set_te_points_number(
            points_number[0]
        ) if self.is_main_blade_active else None
        self.splitter.set_te_points_number(
            points_number[1]
        ) if self.is_splitter_active else None
        self.diffuser.set_te_points_number(
            points_number[2]
        ) if self.is_diffuser_active else None
        ...
        if os.path.exists(self.geomturbo_path):
            ...
            if self.is_channel_active:
                (
                    self.channel.hub,
                    self.channel.shroud,
                ) = inputFile.exportZRNpyArraysList()
    ...
    def set_control_points(self, tag: str, control_points: np.ndarray):
        if tag == "main_blade":
            ...
            print(
                f'Invalid tag: {tag}. Valid tags: "main_blade", "splitter", "diffuser", "hub", "shroud"'
            )

    def fit_compressor_with_bezier(self):
        self.main_blade_control_points = (
            self.main_blade.fit_blade_with_bezier()
            if self.is_main_blade_active
            else None
        )
        self.splitter_control_points = (
            self.splitter.fit_blade_with_bezier() if self.is_splitter_active else None
        )
        self.diffuser_control_points = (
            self.diffuser.fit_blade_with_bezier() if self.is_diffuser_active else None
        )
